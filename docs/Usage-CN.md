...
本文档主要介绍百度智能云JAVA语言版的开发者工具包（SDK），用户可基于该SDK使用JAVA语言接入百度智能云的各项产品（详见支持产品列表）。
...
```text
baiducloud-java-sdk
├── baiducloud-java-sdk-core                  //BCE签名、Http、内部工具相关module
...
├── baiducloud-java-sdk-{产品}                      //产品module
├──├──src/main/java/com.baidubce.{产品}
├──├──├──models                                    //产品类
├──├──├──sample                                    //产品示例代码
├──├──├──{产品}Client.java                          //Client调用
```
...
在使用SDK之前，需确认您将接入的百度智能云产品的Endpoint（服务域名）。以百度智能云VPC产品为例，可阅读[VPC访问域名](https://cloud.baidu.com/doc/VPC/s/xjwvyuhpw)的部分，理解Endpoint相关的概念。其他服务类似，需理解并确认对应服务的Endpoint。
...
下面以百度智能云VPC（私有网络）为例，给出一个基本的使用示例，详细使用说明请参考各服务的详细说明文档。
...
在上面代码中，变量`AK`与`SK`是由系统分配给用户的，均为字符串，用于标识用户，为访问VPC做签名验证。 其中`AK`对应控制台中的“Access Key ID”，`SK`对应控制台中的“Access Key Secret”，获取方式请参考《操作指南 [获取ACCESSKEY](https://cloud.baidu.com/doc/Reference/s/9jwvz2egb)》。

上面的方式使用默认域名作为VPC的服务地址，如果用户需要自己指定域名，可以通过传入ENDPOINT参数来指定。
...
> **注意：** ENDPOINT参数只能用指定的包含区域的域名来进行定义。百度智能云目前开放了多区域支持，请参考[区域选择说明](https://cloud.baidu.com/doc/Reference/s/2jwvz23xx)。

## 配置

### 使用HTTPS协议

该SDK支持使用HTTPS协议访问百度智能云的服务产品，您可以通过如下两种方式在VPC Java SDK中使用HTTPS访问VPC服务：
...
如果用户需要配置VpcClient的一些细节的参数，可以在构造VpcClient的时候传入BceClientConfiguration对象。 BceClientConfiguration是服务的公用配置类，可以为客户端配置代理，最大连接数等参数。
...
VPC Java SDK发布版本中增加了logback作为slf4j的实现，如果用户没有自己的实现可以直接用，如果工程中有其他的如log4j则可以替代。
